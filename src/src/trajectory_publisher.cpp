      ...
      needs_replan_(false)
{
    ...
    // 自动接收 VINS-Fusion 的 Odometry 真实位置
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/vins_estimator/odometry",  // <- 如果你的VINS话题名字不一样，请改这里
        10,
...
void TrajectoryAndObstaclesPublisher::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
    ...
    // 1. 获取小车真实位置 (强制 Z 轴高度为 0，适应 2D 规划)
    cur_pose_.x = msg->pose.pose.position.x;
    cur_pose_.y = msg->pose.pose.position.y;
    
    // Ego-Planner 2D 这里的 z 实际上用来存 yaw(偏航角)
    tf2::Quaternion q(
        msg->pose.pose.orientation.x,
        msg->pose.pose.orientation.y,
        msg->pose.pose.orientation.z,
